...
# Input: 각 환자별 T/N protein log2 fold change 정보가 있는 csv 파일 (외부 파일)
# Output: 해당 정보가 정리된 matrix (protein_matrix)


# 파일 경로 지정
file_path <- "C:/Users/admin/Desktop/LAIDD/RNAseq/Processed proteomics data/Proteomics/protein_expression_change.csv"

# Proteomics 데이터: txt 파일 > csv 파일로 변환 (구분자 tab), T/N log2(fold-change) 정보 있음
# 행렬로 데이터 불러오기
data_frame <- read.csv(file_path, header = TRUE, stringsAsFactors=FALSE)
protein_matrix <- as.matrix(data_frame[, -1])


#-----------------------------------------------------------------------------#


### 2. 50% 이상의 환자에서 결측치가 아닌 protein만 남기고 삭제 / quantile normalization ###


# Input: 각 환자별 T/N protein log2 fold change 정보가 있는 matrix (protein_matrix)  
# Output: 50% 이상의 환자에서 결측치가 아닌 protein 정보 > quantile normalized matrix (norm_p50_matrix)
...
# 각 행(protein)에 대해 NA가 아닌 값이 50% 이상인 행(protein)만 추출
p50_matrix <- protein_matrix[rowMeans(!is.na(protein_matrix[, 6:85])) >= 0.5, ]

# Quantile normalization은 전체 데이터를 모두 일괄로 처리해야함

# protein 정보 빼고 발현량만 추출 후 numeric으로 변환
p50 <- p50_matrix[, 6:85]
...
# 다시 protein 정보 합치기
colnames(norm_p50) <- colnames(p50_matrix)[6:85]
norm_p50_matrix <- cbind(p50_matrix[, 1:5], norm_p50)


#-----------------------------------------------------------------------------#


### Option: 결측치 처리 - mice 패키지 이용: 실제로는 사용하지 않음 ###
...
complete_norm_p50 <- as.matrix(complete_norm_p50_df)


#-----------------------------------------------------------------------------#


### 3. Median absolute deviation (MAD) 기준으로 상위 10%, 20%, 30%  해당하는 protein 추출 ###


# Input: 50% 이상의 환자에서 결측치가 아닌 protein 정보 > quantile normalized matrix (norm_p50_matrix)
# Output: MAD값 기준 상위 10/20/30%에 해당하는 protein 정보만 담긴 matrix, protein name은 제외 (M10/M20/M30)
...
# ExecuteCNMF에 사용할 수 있는 데이터 포맷으로 변경: matrix - 각 행에 ptn, 각 열에 환자
...
# Input: MAD값 기준 상위 10/20/30%에 해당하는 protein 정보만 담긴 matrix (M10/M20/M30)
...
# k=2부터 6까지 반복 실행 (MAD 10%, 20%, 30% 이상 proteins)
for (k in 2:6) {
...
# 위 결과를 통해 MAD 10% 이상 protein들에 대해 cluster 4개로 구분하는 것으로 결정

# Group 정보 / Distance 정보 / Silhouette width / Consensus 정보 저장
group10_4 = result10_list[["result10_4"]][["group"]] #그룹핑 정보
distanceMatrix10_4=result10_list[["result10_4"]][["distanceMatrix"]] #Distance 정보
silhouette10_4=silhouette_SimilarityMatrix(group10_4, distanceMatrix10_4) #Silhouette score
Consensus10_4=result10_list[["result10_4"]][["originalResult"]]@consensus #Consensus 정보

BiocManager::install("ComplexHeatmap")
library(ComplexHeatmap)

# 간단히 Consensus matrix plotting
Heatmap(Consensus10_4,name="Consensus value", show_column_names = FALSE, show_row_names = FALSE)

# Silhouette width plot
sil_plot <- plot(silhouette10_4, col="darkorange3")

# Cophenetic correlation 계산
d1 <- dist(distanceMatrix10_4)
...
# Input: silhouette positive patients 정보 (positive_sil_matrix), 전체 protein에 대한log2(TPM+1) fold change 정보 (f2c_analysis)
# Output: 각 환자 cluster/non-cluster별 분리된 protein expression 데이터 (f2c_pts_1~4/f2c_pts_not1~4)


# Silhouette score가 양수인 환재 샘플 = 해당 subtype에 consensus가 잘 되어있는 환자만 분석에 사용

# silhouette 정보 matrix 생성
sil_matrix <- as.matrix(silhouette10_4[, 1:3])
...
cluster_4_pts <- positive_sil_matrix[clusters == 4, ]
pts_4 <- cluster_4_pts[, "pts"]


# 환자 cluster별 protein expression data 분리
f2c <- norm_p50_matrix #protein name 포함한 전체 protein exp 데이터
f2c_analysis <- norm_p50 #분석용 (protein name 없이 수치만)
...
# protein의 경우 cluster가 4개여서 각 group vs non-group 분석을 실행해야함
...
f2c_pts_4 <- f2c_pts_core[, colnames(f2c_pts_core) %in% pts_4]
f2c_pts_not4 <- f2c_pts_core[, !colnames(f2c_pts_core) %in% pts_4]
...
# Input: 각 환자 cluster/non-cluster별 분리된 protein expression 데이터 (f2c_pts_1~4/f2c_pts_not1~4)
# Output: t_test 결과 with adjusted P value (t_test_group1~4)
# Tool: stats package
...
t_test_group4 <- data.frame(ptn = character(), P_Value = numeric(), stringsAsFactors = FALSE)


# 각 ptn에 대해 group vs non-group t-test 수행 (5924 = 사용할 전체 ptn 수)
for (i in 1:5924) {
  ptn <- i
  
  # 그룹 1내 모든 환자에 대해 t-test 수행
...
for (i in 1:5924) {
  ...
  # 그룹 4내 모든 환자에 대해 t-test 수행
  n_group <- sum(!is.na(f2c_pts_4[i, ]))
  n_not_group <- sum(!is.na(f2c_pts_not4[i, ]))
  
  # 충분한 데이터가 있는 경우에만 t-test 수행하도록 설정 (결측치가 너무 많으면 수행 불가)
  if (n_group >= 2 & n_not_group >= 2) {
    t_result_group4 <- t.test(f2c_pts_4[i, ], f2c_pts_not4[i, ], na.action = na.omit)
  
  # 결과 저장
  t_test_group4 <- rbind(t_test_group4, data.frame(ptn = ptn, P_Value = t_result_group4$p.value))
  }
}
...
t_test_group4$Adjusted_P_Value <- p.adjust(t_test_group4$P_Value, method = "bonferroni")
...
hist(t_test_group4$P_Value, 100)
hist(t_test_group4$Adjusted_P_Value, 100)


#-----------------------------------------------------------------------------#


### 3. Adjusted P value와 median 값 기준으로 각 그룹별 발현되는 proteins 결정 ###


# Input: t_test 결과 with adjusted P value (t_test_group1~4)
# Output: adjusted P value와 median 기준으로 필터링된 signature protein 정보 (f2c_DEG_med_combined)
...
# Adjusted P Value가 0.05 이하인 gene 추출
...
sig_ptn_group3 <- t_test_group3[t_test_group3$Adjusted_P_Value <= 0.05, ]
sig_ptn_group4 <- t_test_group4[t_test_group4$Adjusted_P_Value <= 0.05, ]

# significant_ptn에 해당되는 protein expression data만 추출: name 포함
f2c_DEP_name1 <- f2c[sig_ptn_group1$ptn, ]
f2c_DEP_name2 <- f2c[sig_ptn_group2$ptn, ]
f2c_DEP_name3 <- f2c[sig_ptn_group3$ptn, ]
f2c_DEP_name4 <- f2c[sig_ptn_group4$ptn, ]

# Core pts의 significant_ptn에 해당되는 protein expression data만 추출: name 미포함
...
f2c_DEP_group4 <- f2c_pts_core[sig_ptn_group4$ptn, ]

# Group/non-group별 significan ptn expression data 분리
...
f2c_DEP_pts_4 <- f2c_DEP_group4[ ,colnames(f2c_pts_core) %in% pts_4]
f2c_DEP_pts_not4 <- f2c_DEP_group4[ ,!colnames(f2c_pts_core) %in% pts_4]

# 각 그룹에서 protein별 median 값 추출
...
median_vals_pts_4 <- apply(f2c_DEP_pts_4, 1, median, na.rm=TRUE)
median_vals_pts_not4 <- apply(f2c_DEP_pts_not4, 1, median, na.rm=TRUE)

# 해당 median 값만 비교하기 위해 f2c_median 매트릭스 생성
g1_median <- cbind(f2c_DEP_name1[ ,1:5], g1_median = median_vals_pts_1, not1_median = median_vals_pts_not1)
g2_median <- cbind(f2c_DEP_name2[ ,1:5], g2_median = median_vals_pts_2, not2_median = median_vals_pts_not2)
g3_median <- cbind(f2c_DEP_name3[ ,1:5], g3_median = median_vals_pts_3, not3_median = median_vals_pts_not3)
g4_median <- cbind(f2c_DEP_name4[ ,1:5], g4_median = median_vals_pts_4, not4_median = median_vals_pts_not4)

# 데이터프레임 변환
g1_median_df <- as.data.frame(g1_median)
g2_median_df <- as.data.frame(g2_median)
g3_median_df <- as.data.frame(g3_median)
g4_median_df <- as.data.frame(g4_median)
...
g4_median_df$g4_median <- as.numeric(as.character(g4_median_df$g4_median))
g4_median_df$not4_median <- as.numeric(as.character(g4_median_df$not4_median))

# dplyr 패키지 로드
library(dplyr)

# 각 그룹에서 median 값이 0보다 크고, 타 그룹에서 0보다 작으며, 타 그룹보다 큰 ptn 선별
...
filtered_median_g4 <- g4_median_df %>%
  filter(g4_median > 0 & not4_median < 0 & g4_median > not4_median)

# Protein name이 #N/A 또는 공백이 아닌 행만 선택
filtered_median_g1 <- filtered_median_g1[filtered_median_g1$Full.ID !="#N/A" & filtered_median_g1$Full.ID != "", , drop = FALSE]
filtered_median_g2 <- filtered_median_g2[filtered_median_g2$Full.ID !="#N/A" & filtered_median_g2$Full.ID != "", , drop = FALSE]
filtered_median_g3 <- filtered_median_g3[filtered_median_g3$Full.ID !="#N/A" & filtered_median_g3$Full.ID != "", , drop = FALSE]
filtered_median_g4 <- filtered_median_g4[filtered_median_g4$Full.ID !="#N/A" & filtered_median_g4$Full.ID != "", , drop = FALSE]

# 각 filtered_median_g1~4의 ptn 이름 추출
ptn_vector_p1 <- as.character(unique(filtered_median_g1$Full.ID))
ptn_vector_p2 <- as.character(unique(filtered_median_g2$Full.ID))
ptn_vector_p3 <- as.character(unique(filtered_median_g3$Full.ID))
ptn_vector_p4 <- as.character(unique(filtered_median_g4$Full.ID))
...
pts_3 <- cluster_3_pts[, "pts"]
cluster_4_pts <- sil_matrix[clusters == 4, ]
pts_4 <- cluster_4_pts[, "pts"]

# f2c에서 Full_ID가 ptn_vector와 일치하는 행 선택
...
ptn_DEG_med_p4 <- f2c[f2c[, 1] %in% ptn_vector_p4, ]

# Heatmap용 matrix 생성
# 이 때, 환자수가 많은 그룹부터 배열하여 consensus 맵 배열과 일치시키자
# 환자수가 많은 순으로 정렬
ptn_DEG_med_p4132 <- rbind(ptn_DEG_med_p4, ptn_DEG_med_p1, ptn_DEG_med_p3, ptn_DEG_med_p2)

f2c_DEG_med_pts1 <- ptn_DEG_med_p4132[,colnames(ptn_DEG_med_p4132) %in% pts_1]
f2c_DEG_med_pts2 <- ptn_DEG_med_p4132[,colnames(ptn_DEG_med_p4132) %in% pts_2]
f2c_DEG_med_pts3 <- ptn_DEG_med_p4132[,colnames(ptn_DEG_med_p4132) %in% pts_3]
f2c_DEG_med_pts4 <- ptn_DEG_med_p4132[,colnames(ptn_DEG_med_p4132) %in% pts_4]

f2c_DEG_med_combined <- cbind(f2c_DEG_med_pts4, f2c_DEG_med_pts1, f2c_DEG_med_pts3, f2c_DEG_med_pts2)
f2c_DEG_med_combined <- apply(f2c_DEG_med_combined, 2, as.numeric)

# NA를 0으로
f2c_DEG_med_combined[is.na(f2c_DEG_med_combined)] <- 0

# Protein: pts_4 = group 1, pts_1 = group 2, pts_3 = group 3, pts_2 = group 4
...
# Input: 필터링된 signature protein 정보 (f2c_DEG_med_combined), 그룹 정보 (sil_matrix$cluster)
# Output: Heatmap 그룹핑에 사용할 annotation 정보 (csv 파일)


# Cluster
write.csv(colnames(f2c_DEG_med_combined), "ptn_subgroup_cluster.csv", row.names = TRUE)
# 파일 추출 후에 group별 환자수만큼 1열에 PTN1, PTN2, PTN3, PTN4 기입
# 1행에는 Cluster 기입

# Signature proteins: Signature molecule의 row annotation에 사용
write.csv(f2c_DEG_med_combined[,1], "ptn_subgroup_signature.csv", row.names = TRUE)
# 파일 추출 후에 group별 signature protein수만큼 1열에 Ptn1, Ptn2, Ptn3, Ptn4 기입
# 1행에는 Signature 기입

# Consensus
write.csv(sil_matrix[,1:2], "ptn_subgroup_consensus.csv", row.names = TRUE)
# 파일 추출 후에 group별 환자에 맞게 PTN1, PTN2, PTN3, PTN4 기입
# 1행에는 Consensus 기입
...
# Input: signature protein 정보 (f2c_DEG_med_combined), consensus 정보 (Consensus10_4), annotation 파일들
# Output: Signature protein heatmap (heatmap1), Consensus heatmap (heatmap2) 
...
anno_C = read.csv("C:/Users/admin/Desktop/LAIDD/RNAseq/Clustering/Subgroup/ptn_subgroup_cluster.csv")
ha_C=HeatmapAnnotation(df = anno_C, which="col", col = list(
  Cluster = c("PTN1" = "deeppink1", "PTN2" = "deeppink4", "PTN3" = "blue4", "PTN4" = "dodgerblue3")),
  show_annotation_name = FALSE, show_legend = FALSE
)

# Signature annotation (row)
anno_S = read.csv("C:/Users/admin/Desktop/LAIDD/RNAseq/Clustering/Subgroup/ptn_subgroup_signature.csv")
ha_S=HeatmapAnnotation(df = anno_S, which="row", col = list(
  Signature = c("Ptn1" = "deeppink1", "Ptn2" = "deeppink4", "Ptn3" = "blue4", "Ptn4" = "dodgerblue3")),
  show_annotation_name = FALSE, show_legend = FALSE
)
...
anno_A = read.csv("C:/Users/admin/Desktop/LAIDD/RNAseq/Clustering/Subgroup/ptn_subgroup_consensus.csv")
ha_A=HeatmapAnnotation(df = anno_A, which="col", col = list(
  Consensus = c("PTN1" = "deeppink1", "PTN2" = "deeppink4", "PTN3" = "blue4", "PTN4" = "dodgerblue3" )),
show_annotation_name = FALSE, show_legend = FALSE
)

# Heatmap
heatmap2 = Heatmap(Consensus10_4,name="Consensus value",
                   show_column_names = FALSE,
                   show_row_names = FALSE,
...
# Output: group에 속한 여부가 1 또는 0으로 표현된 indicative matrix (indi_ptn.csv) 
...
# 행 추가
t_sil_matrix <- rbind(t_sil_matrix, NA, NA)
...
indi_ptn <- t_sil_matrix
rownames(indi_ptn)[1:4] <- c("PTN2", "PTN4", "PTN3", "PTN1")

# indicator matrix를 csv로 저장
write.csv(indi_ptn, "indi_ptn.csv", row.names = TRUE)
